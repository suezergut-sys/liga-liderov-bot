...
- четыре этапа финансовой игры: начало Q1, начало Q2, начало Q3 и конец Q3;
- отдельные связанные сценарии для красных команд («Якорь») и синих команд («Шахта», «Вышка», «Птица»);
- последствия решений предыдущих кварталов в следующих ситуациях, включая найм, подряд, PR-расходы и аванс бонуса;
...
Завершённые изменения публикуются из отдельной ветки через pull request. После успешных обязательных проверок готовый PR автоматически объединяется с `main`, если пользователь явно не запретил merge для конкретной задачи; затем проверяются production deployment и живой URL.
...
src/lib/scenario.ts     четыре этапа и ветвления красного и синего сценариев
...
- `Ситуации для игры_final.xmind` — актуальный источник реализованных командных сценариев.

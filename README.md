...
5. Бот показывает выбранный вариант и кнопку **«Подтвердить»**. После подтверждения изменить решение нельзя.
6. Бот просит отправить обновлённый бюджет. Принимается только файл `.xlsx` размером до 20 МБ.
7. После сохранения файла команда получает статус **«Готова»** и ждёт открытия следующего этапа.
8. На новом этапе бот формирует ситуацию с учётом истории подтверждённых решений этой команды.
...
| 2. Начало Q2 | Две сотрудницы уходят в декрет, а модернизация «Якоря» задерживается. Команда выбирает одну из четырёх комбинаций: найм или подрядчики для «Гаммы» × старт модернизации сейчас или в Q3. Последствия Q1 добавляются в текст ситуации. | Проект «Вышки» на 56 млн рублей при загрузке персонала 90%. Команда одновременно принимает три решения: нанимать ли двух СК, проводить ли PR за 15 млн и выдавать ли продавцу 30% бонуса авансом. Восемь комбинаций сохраняются как один аудируемый выбор. |
| 3. Начало Q3 | «Якорь» не повышает ставки поддержки, зарплаты растут на 10%, другие клиенты повышают ставки на 5%. Команда отвечает «Да» или «Нет» на вопрос, будет ли она что-то менять в Прогнозе года, чтобы выполнить цели Бюджета, подтверждает ответ и загружает актуальный `.xlsx`. | Трудоёмкость и сроки «Вышки» недооценены вдвое. Выбор: остановить исходный проект либо продолжить за свой счёт. Подпись последствия зависит от PR и аванса бонуса в Q2. |
| 4. Конец Q3 | Новый клиент просит скидку 10% и не допускает подряд. Доступность ресурсов зависит от найма в Q2. Выбор: дать скидку и взять клиента либо отказаться. | Последствия по «Вышке» и продавцу определяют, появляется ли «Птица» и может ли команда добавить 7 млн рублей выручки. Команда фиксирует последствия в финансовом прогнозе. |
...
stateDiagram-v2
    ...
    decision_selected --> awaiting_file: решение подтверждено
...
| `app.decisions` | Неизменяемая аудируемая запись подтверждённого или принудительного решения |
...
- без Blob-токена store сохраняет Telegram `file_id` как ссылку-заглушку — это подходит для demo, но не заменяет постоянное файловое хранилище.
...
- синие решения Q2 представлены восемью составными кнопками, а не тремя последовательными вопросами;
